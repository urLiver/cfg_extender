...
namespace CodGhost
{
	...
	void Cmd_ClearPrints( void )
	{
		...
		{
			LogWrite( "cfge_clear_prints called with arguments, while it shouldnt have any, argc: %i\n", ArgC() );
		}
	}
	...
	void Cmd_Print( void )
	{
		if( ArgC() == 2 )
		{
			Print( cmd_args->localClientNum[ cmd_args->nesting ], ArgV( 1 ) );
		}
		else
		{
			LogWrite( "cfge_print <text> : prints text to the screen, argc: %i\n", ArgC() );
		}
	}
}